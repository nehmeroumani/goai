...
The schema is sent on every step. The model freely interleaves executable tool calls and eventually produces the final JSON output when it is ready. Structured output is parsed from the first step that is not an executable-tool continuation; it does not require finish reason `"stop"`.
...
If every allowed step continues through executable tool calls and `MaxSteps` is exhausted before a structured output step, `GenerateObject` returns an error:

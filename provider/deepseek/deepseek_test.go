...
func TestChat_Stream(t *testing.T) {
	...
	model := Chat("deepseek-chat", WithAPIKey("test-key"), WithBaseURL(server.URL))
...
func TestChat_Generate(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, `{"id":"chatcmpl-123","model":"deepseek-chat","choices":[{"message":{"role":"assistant","content":"Hello world"},"finish_reason":"stop"}],"usage":{"prompt_tokens":10,"completion_tokens":5}}`)
	}))
	defer server.Close()
...
func TestCapabilities(t *testing.T) {
	...
	if !caps.Reasoning {
		t.Error("DeepSeek should support reasoning")
	}
}

func TestModelID(t *testing.T) {
	model := Chat("deepseek-chat", WithAPIKey("k"))
...
func TestChat_PromptCachingIgnored(t *testing.T) {
	...
	genServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = fmt.Fprint(w, `{"id":"chatcmpl-test","model":"deepseek-chat","choices":[{"index":0,"message":{"role":"assistant","content":"ok"},"finish_reason":"stop"}],"usage":{"prompt_tokens":5,"completion_tokens":2,"total_tokens":7}}`)
	}))
	defer genServer.Close()

	genModel := Chat("deepseek-chat", WithAPIKey("test-key"), WithBaseURL(genServer.URL))
	...
	streamModel := Chat("deepseek-chat", WithAPIKey("test-key"), WithBaseURL(streamServer.URL))
...
// Item 58: DeepSeek's top-level prompt_cache_hit_tokens / prompt_cache_miss_tokens
// must be parsed into usage (CacheReadTokens = hit, InputTokens = miss).
func TestChat_ParsesTopLevelCacheCounters(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = fmt.Fprint(w, `{"id":"chatcmpl-1","model":"deepseek-chat","choices":[{"index":0,"message":{"role":"assistant","content":"ok"},"finish_reason":"stop"}],"usage":{"prompt_tokens":100,"completion_tokens":10,"prompt_cache_hit_tokens":70,"prompt_cache_miss_tokens":30}}`)
	}))
	defer server.Close()

	model := Chat("deepseek-chat", WithAPIKey("k"), WithBaseURL(server.URL))
	result, err := model.DoGenerate(t.Context(), provider.GenerateParams{
		Messages: []provider.Message{{Role: provider.RoleUser, Content: []provider.Part{{Type: provider.PartText, Text: "hi"}}}},
	})
	if err != nil {
...
// Item 47: deepseek opts into IncludeReasoningContent, so an assistant message
// carrying a reasoning part must serialize reasoning_content on the outgoing
// request body (DeepSeek 400s if a thinking-mode assistant message omits it).

...
func TestChat_Stream(t *testing.T) {
	...
	model := Chat("meta-llama/Llama-3.3-70B-Instruct-Turbo", WithAPIKey("test-key"), WithBaseURL(server.URL))
...
func TestChat_Generate(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, `{"id":"chatcmpl-123","model":"meta-llama/Llama-3.3-70B-Instruct-Turbo","choices":[{"message":{"role":"assistant","content":"Hello world"},"finish_reason":"stop"}],"usage":{"prompt_tokens":10,"completion_tokens":5}}`)
	}))
	defer server.Close()
...
func TestModelID(t *testing.T) {
	model := Chat("meta-llama/Llama-3.3-70B-Instruct-Turbo", WithAPIKey("k"))
	if model.ModelID() != "meta-llama/Llama-3.3-70B-Instruct-Turbo" {
